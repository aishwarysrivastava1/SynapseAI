...
import { Users, ArrowLeft, MapPin, CheckCircle, Zap, Shield, AlertCircle } from "lucide-react";
...
export default function VolunteerLoginPage() {
  ...
  useEffect(() => {
    ...
    if (role === "Volunteer") {
      ...
      // New user — assign Volunteer role
      roleAssigned.current = true;
      setRedirecting(true);
      setUserRole("Volunteer")
        .then(() => router.replace("/volunteer-dashboard"))
        .catch((err) => {
    ...
    return (
      <div className="min-h-screen bg-[#F5F6F1] dark:bg-gray-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-4 border-[#48A15E] border-t-transparent rounded-full animate-spin" />
  ...
  return (
    <div className="min-h-screen bg-[#F5F6F1] dark:bg-gray-950 flex flex-col relative overflow-hidden">
      ...
      {/* Ambient glows */}
      <div className="pointer-events-none absolute top-[-15%] left-[-10%] w-[50%] h-[50%] rounded-full bg-[#48A15E]/6 dark:bg-[#48A15E]/10 blur-3xl" />
      <div className="pointer-events-none absolute bottom-[-20%] right-[-10%] w-[45%] h-[45%] rounded-full bg-[#115E54]/5 dark:bg-[#115E54]/8 blur-3xl" />
        ...
        <div className="w-full max-w-4xl lg:max-w-5xl grid md:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="text-center md:text-left animate-slide-up">
            <div className="inline-flex items-center justify-center md:justify-start mb-5">
              <div className="w-14 h-14 rounded-2xl bg-[#48A15E]/12 dark:bg-[#48A15E]/20 flex items-center justify-center">
                <Users size={28} className="text-[#48A15E]" />
              </div>
            </div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed mb-8">
              Join the frontline. Claim tasks, submit verified proof, and earn XP while making a real difference during emergencies.
            </p>
            <ul className="space-y-3 text-left">
              {FEATURES.map(({ icon: Icon, text }) => (
                <li key={text} className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                  <div className="w-7 h-7 rounded-lg bg-[#48A15E]/8 dark:bg-[#48A15E]/15 flex items-center justify-center shrink-0">
          ...
          <div className="animate-slide-up delay-100">
            <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-sm border border-gray-200 dark:border-gray-800 flex flex-col gap-5">
              <div className="text-center">
                ...
                <h2 className="font-bold text-gray-900 dark:text-gray-100 text-base">Sign in to your volunteer account</h2>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                  First sign-in? Your account is set up as Volunteer automatically.
                </p>
              </div>
              ...
              <p className="text-center text-xs text-gray-400 dark:text-gray-600">
                Are you an NGO?{" "}
                <Link href="/login/ngo" className="text-[#115E54] hover:underline font-medium">
                  NGO sign-in →
                </Link>
              </p>
            </div>
