...
export default function SimulationPanel() {
  ...
  const handleSimCompare = async () => {
    setLoading(true);
    const result = await runComparisonSim(100);
    if (result) {
  ...
  const handleAskGraph = async () => {
    ...
    setLoading(true);
    try {
      const result = await askGraphIntelligence(query);
      setAskResult(result);
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="absolute bottom-6 left-6 right-6 hud-panel border border-neon-cyan/20 rounded-2xl p-5 shadow-[0_0_40px_rgba(0,0,0,0.8),inset_0_0_20px_rgba(0,243,255,0.05)] flex gap-8 z-20 group max-h-72">
         ...
         <div className="flex gap-3">
            <div className="relative flex-1 group-hover:shadow-[0_0_15px_rgba(0,243,255,0.15)] transition-all rounded-lg">
              ...
              <input 
                ...
                className="w-full bg-slate-950/80 border border-slate-800 rounded-lg py-2 pl-8 pr-4 text-sm text-neon-cyan focus:outline-none focus:border-neon-cyan/50 font-mono placeholder-slate-600 shadow-[inset_0_0_10px_rgba(0,0,0,0.5)]"
                value={query}
      ...
      <div className="w-[400px] flex flex-col min-w-[380px]">
        ...
        <button 
          ...
          disabled={loading}
          className="bg-neon-purple/10 hover:bg-neon-purple/20 text-neon-purple border border-neon-purple/50 px-4 py-2.5 rounded-lg text-xs font-bold transition-all mb-4 tracking-widest uppercase hover:shadow-[0_0_15px_rgba(217,0,255,0.2)]"
        >
          {loading ? "Simulating Scenarios..." : "Run Parallel Comparison"}
        </button>
