    ...
    async def post(self, request):
        ...
        async def event_generator():
            try:
                ...
                if not budget_ok:
                    payload = json.dumps({"error": "Daily token budget exceeded. Try again tomorrow."})
                ...
                if not safe:
                    payload = json.dumps({"error": f"Message blocked: {reason}"})
                    yield f"data: {payload}\n\n"
                    return
                ...
                if cached:
                    for word in cached.get("reply_text", "").split():
                        yield f"data: {json.dumps({'textChunk': word + ' '})}\n\n"
                    yield f"data: {json.dumps({'done': True, 'action_response': cached.get('action_response', {})})}\n\n"
                    return
                ...
                async for chunk in orchestrator.stream(
                    ...
                    if isinstance(chunk, dict):
                        ...
                        full_reply += chunk
                        yield f"data: {json.dumps({'textChunk': chunk})}\n\n"
                ...
                yield f"data: {json.dumps({'done': True, 'action_response': action_response})}\n\n"
